...
class AddRefundAccount extends Component {
  ...
  componentWillReceiveProps(props) {
    if (props.bank) {
      ...
      if (props.bank.error) {
        ...
        this.setState({ error: { text: null, status: null } })
  ...
  handleSubmit(values) {
	...
	render() {
        ...
        <div className="row">
          <div className="col-12">
            <p className={cn('inline-headers info text-center', this.state.error.status ? 'text-danger' : null)}>
  ...
  renderField(field) {
    ...
    const className = `form-group ${touched && error ? 'has-warning' : ''} ${valid ? 'has-success' : ''}  ${valid ? 'has-warning' : ''}`
...
export default reduxForm({
  ...
  connect(({ bank }) => ({ bank }), { addAccount })(AddRefundAccount)
)
